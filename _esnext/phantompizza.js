...
<div id="start-screen">
  <h1 id="start-screen-title">${U}</h1>
  <p id="start-screen-text">${a}</p>
</div>

<style>
  :root {
    --phantom-background-color: ${V};
    --phantom-foreground-color: ${Y};
  }
...
`;let A=S.reduce((n,r)=>({...n,[r.split("/").pop().split(".")[0]]:document.getElementById(r.split("/").pop().split(".")[0])}),{score:document.getElementById("score")}),f=document.getElementById("start-screen"),H=document.getElementById("start-screen-text"),l=document.querySelector("canvas");l.width=innerWidth,l.height=innerHeight;let o={x:l.width/2,y:l.height/2},G={value:0,sprite:A.score},X=q.reduce((n,r)=>{let I=new Audio(r.url);return I.volume=r.volume,{...n,[r.url.split("/").pop().split(".")[0]]:I}},{});return setInterval(()=>H.innerHTML=H.innerHTML===a?m:a,2e3),addEventListener("resize",()=>(l.width=innerWidth,l.height=innerHeight)),addEventListener("mousemove",({clientX:n,clientY:r})=>(o.x=n,o.y=r)),addEventListener("touchmove",n=>(o.x=n.touches[0].clientX-17,o.y=n.touches[0].clientY-90),{passive:!1}),addEventListener("touchstart",n=>(o.x=n.touches[0].clientX,o.y=n.touches[0].clientY),{passive:!1}),addEventListener("keydown",n=>n.key==="Enter"&&f.remove()),addEventListener("keydown",n=>n.key==="ArrowUp"&&(o.y-=60)),addEventListener("keydown",n=>n.key==="ArrowRight"&&(o.x+=60)),addEventListener("keydown",n=>n.key==="ArrowDown"&&(o.y+=60)),addEventListener("keydown",n=>n.key==="ArrowLeft"&&(o.x-=60)),addEventListener("click",()=>f.remove()),addEventListener("touchstart",()=>f.remove()),{c:l,mouse:o,sound:X,score:G,sprites:A}};var P=/iPhone|iPad|iPod|Android/i.test(navigator.userAgent);if(P){let e=document.createElement("style");e.textContent=`
    #main {
      display: flex;
      justify-content: center;
      align-items: center;
